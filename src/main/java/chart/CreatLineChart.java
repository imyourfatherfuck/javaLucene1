...
public class CreatLineChart {
     ...
     * @throws Exception
     */
    public static void CreateNewLineChartForPng1(String title, String xtitle, String ytitle, String filepath, List<String> categorie, List<Serie> series, int width, int height) throws Exception {
        ChartPanel chartPanel = new CreatLineChart().createChart1(title, xtitle, ytitle, categorie, series);
        //将图片保存为png格式
        saveAsFile(chartPanel.getChart(), filepath, width, height);
    }
        ...
        for (Serie serie : series) {
            ...
            if (data != null && categories != null && data.size() == categories.length) {
                for (int index = 0; index < data.size(); index++) {
                    ...
                    if (isNumber(value)) {
                        dataset.setValue(Integer.valueOf(value), name, categories[index]);
                    }
                }
            }
     ...
     */
    @SuppressWarnings("deprecation")
    public static void setLineRender1(CategoryPlot plot, boolean isShowDataLabels, boolean isShapesVisible) {
        plot.setNoDataMessage(NO_DATA_MSG);
        plot.setInsets(new RectangleInsets(10, 10, 0, 10), false);
        plot.setDomainGridlinesVisible(true); // 显示横坐标格线
        plot.setRangeGridlinesVisible(true); // 显示纵坐标格线
        LineAndShapeRenderer renderer = (LineAndShapeRenderer) plot.getRenderer();
        DecimalFormat decimalformat1 = new DecimalFormat(" ##.## "); // 数据点显示数据值的格式
        renderer.setItemLabelGenerator(new StandardCategoryItemLabelGenerator(" {2} " + "name", decimalformat1));
        // 上面这句是设置数据项标签的生成器
        renderer.setItemLabelsVisible(true); // 设置项标签显示
        renderer.setBaseItemLabelsVisible(true); // 基本项标签显示
...
//           series.add(new Serie("Berlin", new Double[] { 42.4, 33.2, 34.5, 39.7, 52.6, 75.5, 57.4, 60.4, 47.6, 39.1, 46.8, 51.1 }));
     ...
     * @throws Exception
     */
    public ChartPanel createChart1(String title, String xtitle, String ytitle, List<String> categorie, List<Serie> series) throws Exception {
        ...
        CreatLineChart.setAntiAlias(chart);// 抗锯齿
        // 4:对柱子进行渲染[[采用不同渲染]]
        CreatLineChart.setLineRender1(chart.getCategoryPlot(), false, true);//
        // 5:对其他部分进行渲染
        CreatLineChart.setXAixs(chart.getCategoryPlot());// X坐标轴渲染
        CreatLineChart.setYAixs(chart.getCategoryPlot());// Y坐标轴渲染
        // 设置标注无边框
...
//            saveAsFile(chartPanel.getChart(),"D:\\1\\lol.png",900,500);
            CreateNewLineChartForPng("折线图.png", "X轴", "Y轴", "E:\\lucene\\折线图.png", categorie, series, 900, 500);
